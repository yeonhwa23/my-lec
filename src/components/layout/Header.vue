...
const menuItems = [
  ...
  { key: 'workspace', name: '워크스페이스', children: [
    { name: '채널 목록', path: '/' }, { name: '내 워크스페이스', path: '/' },
    { type: 'divider' }, { name: '새 워크스페이스 만들기', path: '/' },
  ]},
  { key: 'community', name: '커뮤니티', children: [
    { name: '방명록', path: '/guest' }, { name: '자유 게시판', path: '/board' }, { name: '포토 갤러리', path: '/photo' },
  ]},
  { key: 'customer', name: '고객센터', children: [
    { name: '자주 묻는 질문', path: '/faq' }, { name: '공지사항', path: '/notice' }, { name: '1:1 문의', path: '/inquiry' },
  ]},
];
            ...
            <li v-else class="wh-li has-drop" @mouseenter="openMenu(item.key)" @mouseleave="scheduleClose">
              ...
              <div class="drop" :class="{ show: pcOpenKey === item.key }" @mouseenter="cancelClose" @mouseleave="scheduleClose">
                <template v-for="(sub, i) in item.children" :key="i">
                  ...
                  <router-link v-else :to="sub.routeName ? { name: sub.routeName } : sub.path" class="dlink" @click="closeAll">{{ sub.name }}</router-link>
                </template>
              </div>
            </li>
          </template>
...
.wh-mark {
  ...
  background: linear-gradient(145deg, #9A9BD0, #7879C0, #6667AB);
  display: flex; align-items: center; justify-content: center;
  font-size: 14px; color: #fff;
  box-shadow: 0 0 20px rgba(120, 121, 192, 0.4), inset 0 1px 0 rgba(255,255,255,0.2);
  transition: all 0.3s;
}
...
.dlink { display: block; padding: 10px 14px; margin: 2px 6px; font-size: 13px; font-weight: 400; color: rgba(220, 218, 240, 0.5) !important; border-radius: 9px; text-decoration: none; transition: all 0.15s; background: transparent !important; letter-spacing: 0.01em; }
...
.btn-pill {
  display: inline-flex; align-items: center; gap: 4px; padding: 8px 20px;
  ...
  box-shadow: 0 0 24px rgba(120, 121, 192, 0.28), inset 0 1px 0 rgba(255,255,255,0.15);
}
